...
/**
 * changes_history - creates a file, or appends to an existing file
...
int changes_history(info_t *info)
{
	...
	{
		_insfd(node->str, fdes);
		_insfd('\n', fdes);
	}
	_insfd(BUF_FLUSH, fdes);
	close(fdes);
	return (1);
}
...
/**
 * changeno_history - renumbers the history linked list after changes
 * @info: Structure containing potential arguments. Used to maintain
 *
