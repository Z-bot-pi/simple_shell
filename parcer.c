...
/**
 * exec_file - determines if a file is an executable command
 * @info: the info struct
 * @path: path to the file
 *
 * Return: 1 if true, 0 otherwise
 */
int exec_file(info_t *info, char *path)
{
	struct stat st;
...
char *get_path(info_t *info, char *pathstr, char *cmd)
{
	...
	while (1)
	{
		...
		{
			path = chars_dup(pathstr, curr_pos, i);
			if (!*path)
				_strcat(path, cmd);
			else
			{
